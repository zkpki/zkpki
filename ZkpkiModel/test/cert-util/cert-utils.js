...
-----END CERTIFICATE-----`;
        ...
        it("BER Array to PEM String",
            async function () {
                // btoa() isn't defined when running this mocha test
                global.btoa = function (str) {
                    const buff = Buffer.from(str, "binary");
                    return buff.toString("base64");
                };
                const converted = certUtil.conversions.berToPem("CERTIFICATE", berArray);
                assert.deepEqual(converted, pemString, "Same BER contents");
            });

        it("PEM String to BER Array",
            async function () {
                // atob() isn't defined when running this mocha test
                global.atob = function (str) {
                    const buff = Buffer.from(str, "base64");
                    return [...buff];
                };
                const converted = certUtil.conversions.pemToBer(pemString);
                assert.deepEqual(converted, berArray, "Same BER contents");
            });
    });
